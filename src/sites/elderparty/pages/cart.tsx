...
import { getProductBySlug } from "@/sites/elderparty/data/products"
import { useSiteLink } from "@/hooks/use-site-link"

const AWAKENING_PROCESSING_FEE = 3.13
const DIMENSIONAL_TRANSIT_TAX_RATE = 0.04

export const metadata = {
  title: "Your Cart — Elder Party",
  description: "Review your order. The Elder Party has been expecting this.",
}

export default function ElderPartyCart() {
  ...
  const dimensionalTax = subtotal * DIMENSIONAL_TRANSIT_TAX_RATE
  const total = subtotal + AWAKENING_PROCESSING_FEE + dimensionalTax
      ...
      <section className="py-20 px-4 text-center">
        <div className="max-w-lg mx-auto">
          ...
          <p className="text-foreground/60 mb-8">
            Your cart is as empty as the void between dimensions.
          </p>
          <Link
            href={siteHref("/shop")}
      ...
      <div className="max-w-4xl mx-auto">
          ...
          {cartItems.map(({ slug, quantity, product }) => (
            <div key={slug} className="py-6 flex gap-4 items-center">
              ...
              <div className="flex-1">
                <Link href={siteHref(`/shop/${slug}`)} className="font-heading font-semibold text-primary hover:underline">
                  {product.name}
                </Link>
                <p className="text-foreground/60 text-sm">{product.priceLabel}</p>
                <p className="text-foreground/40 text-xs font-mono capitalize">{product.category}</p>
              </div>
        ...
        <div className="mt-8 border-t border-primary/10 pt-8">
          <div className="max-w-xs ml-auto space-y-2">
            ...
            <div className="flex justify-between text-foreground/70">
              <span>Awakening Processing Fee</span>
              <span>${AWAKENING_PROCESSING_FEE.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-foreground/70">
              <span>Dimensional Transit Tax (4.0%)</span>
              <span>${dimensionalTax.toFixed(2)}</span>
            </div>
          ...
          <p className="text-xs text-foreground/40 mt-4 text-right">
            * By proceeding to checkout you acknowledge that the Elder Party assumes no liability for dimensional side effects resulting from your purchase.
          </p>
