...
async function main() {
  ...
  console.log(`  GEN   experience-hero.png (refs: ${refs.map((p) => path.basename(p)).join(", ")}) ...`);

  const imageFiles = await Promise.all(refs.map((p) => toImageFile(p)));
  const response = await openai.images.edit({
    model: "gpt-image-1" as any,
    image: imageFiles as any,
    prompt: "A centered group photo of these four men standing near a bathroom door, all looking concerned and worried. Each man is wearing a different colored flannel shirt — red flannel, blue flannel, green flannel, gray flannel. Preserve each person's face and likeness from the reference photos but change their clothing to flannel. The group is centered in the frame with normal proportions. Warm indoor lighting, a closed bathroom door visible behind them. Shot at normal focal length with no distortion.",
    size: "1536x1024",
  });

  const b64 = (response as any).data?.[0]?.b64_json;
  if (!b64) throw new Error("No data");
  writeFileSync(path.join(OUT_DIR, "experience-hero.png"), Buffer.from(b64, "base64"));
