...
- Directory: `src/sites/domainecarter/`
...
- `ogImage` field in `config.ts` metadata, pointing to `/sites/domainecarter/hero.png`
- Static assets live under `public/sites/domainecarter/`
...
Journal data lives in `src/sites/domainecarter/data/journal.ts` with shape `{ slug, title, excerpt, publishedDate, author, body }`.
...
Portraits generated via the existing `scripts/` pipeline using the four reference photos as source. Outputs to `public/sites/domainecarter/exec-<slug>.png` with site-themed styling per the table above.
...
5. **OG image** — `ogImage` field in `config.ts` metadata pointing to `/sites/domainecarter/hero.png`.
6. **Subdomain allowlist** — `domainecarter` added to `VALID_SUBDOMAINS` in `src/sites/subdomains.ts`.
